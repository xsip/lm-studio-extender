...
export class ResponseOutputTextDto {
  ...
  @ApiProperty({ description: `The annotations of the text output.`, isArray: true })
  @IsArray()
  annotations!: FileCitationDto | URLCitationDto | ContainerFileCitationDto | FilePathDto[];

  /** The text output from the model. */
  @ApiProperty({ description: `The text output from the model.`, type: 'string' })
  @IsString()
  text!: string;

  /** The type of the output text. Always `output_text`. */
  @ApiProperty({ description: `The type of the output text. Always \`output_text\`.`, example: 'output_text' })
  @Equals('output_text')
  type!: 'output_text';

  @ApiProperty({ required: false, type: () => [LogprobDto], isArray: true })
  @IsOptional()
  @IsArray()
