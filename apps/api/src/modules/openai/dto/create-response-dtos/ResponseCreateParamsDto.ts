...
export class ResponseCreateParamsDto {
...
for more information. */
  @ApiProperty({ required: false, description: `If set to true, the model response data will be streamed to the client as it is
generated using
[server-sent events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events#Event_stream_format).
See the
[Streaming section below](https://platform.openai.com/docs/api-reference/responses-streaming)
...
[Learn more](https://platform.openai.com/docs/guides/background).` })
  ...
  @ApiProperty({ required: false, description: `Context management configuration for this request.` })
  @IsOptional()
...
response completes. */
  @ApiProperty({ required: false, description: `The conversation that this response belongs to. Items from this conversation are
prepended to \`input_items\` for this response request. Input items and output
items from this response are automatically added to this conversation after this
response completes.` })
  @IsOptional()
...
- `code_interpreter_call.outputs`: Includes the outputs of python code execution
  in code interpreter tool call items. */
  @ApiProperty({ required: false, description: `Specify additional output data to include in the model response. Currently
supported values are:
...
- [Function calling](https://platform.openai.com/docs/guides/function-calling) */
  @ApiProperty({ required: false, description: `Text, image, or file inputs to the model, used to generate a response.
...
swap out system (or developer) messages in new responses. */
  @ApiProperty({ required: false, description: `A system (or developer) message inserted into the model's context.
...
a maximum length of 512 characters. */
  @ApiProperty({ required: false, description: `Set of 16 key-value pairs that can be attached to an object. This can be useful
for storing additional information about the object in a structured format, and
querying for objects via API or the dashboard.
...
available models. */
  @ApiProperty({ required: false, description: `Model ID used to generate the response, like \`gpt-4o\` or \`o3\`. OpenAI offers a
wide range of models with different capabilities, performance characteristics,
and price points. Refer to the
[model guide](https://platform.openai.com/docs/models) to browse and compare
available models.`, type: () => ResponsesModelDto })
  ...
  @ApiProperty({ required: false, description: `Whether to allow the model to run tool calls in parallel.` })
  @IsOptional()
...
utilized.` })
  ...
  @ApiProperty({ required: false, description: `Whether to store the generated model response for later retrieval via API.` })
  @IsOptional()
  store?: null | false | true;

  /** Options for streaming responses. Only set this when you set `stream: true`. */
  @ApiProperty({ required: false, description: `Options for streaming responses. Only set this when you set \`stream: true\`.` })
  @IsOptional()
...
not both. */
  @ApiProperty({ required: false, description: `What sampling temperature to use, between 0 and 2. Higher values like 0.8 will
make the output more random, while lower values like 0.2 will make it more
focused and deterministic. We generally recommend altering this or \`top_p\` but
not both.` })
  @IsOptional()
...
can call. */
  @ApiProperty({ required: false, description: `How the model should select which tool (or tools) to use when generating a
response. See the \`tools\` parameter to see how to specify which tools the model
can call.` })
  @IsOptional()
...
- **Function calls (custom tools)**: Functions that are defined by you, enabling
  ...
  @ApiProperty({ required: false, description: `An array of tools the model may call while generating a response. You can
specify which tool to use by setting the \`tool_choice\` parameter.
...
We generally recommend altering this or `temperature` but not both. */
  @ApiProperty({ required: false, description: `An alternative to sampling with temperature, called nucleus sampling, where the
model considers the results of the tokens with top_p probability mass. So 0.1
means only the tokens comprising the top 10% probability mass are considered.
