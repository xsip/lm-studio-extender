...
export class ResponseOutputMessageDto {
  ...
  @ApiProperty({ description: `The content of the output message.`, isArray: true })
  @IsArray()
  content!: ResponseOutputTextDto | ResponseOutputRefusalDto[];

  /** The role of the output message. Always `assistant`. */
  @ApiProperty({ description: `The role of the output message. Always \`assistant\`.`, example: 'assistant' })
  @Equals('assistant')
  role!: 'assistant';

  /** The status of the message input. One of `in_progress`, `completed`, or
`incomplete`. Populated when input items are returned via API. */
  @ApiProperty({ description: `The status of the message input. One of \`in_progress\`, \`completed\`, or
