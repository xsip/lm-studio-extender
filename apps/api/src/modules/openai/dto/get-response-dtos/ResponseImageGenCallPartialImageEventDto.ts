...
export class ResponseImageGenCallPartialImageEventDto {
  ...
  @ApiProperty({ description: `Base64-encoded partial image data, suitable for rendering as an image.`, type: 'string' })
  @IsString()
  partial_image_b64!: string;

  /** 0-based index for the partial image (backend is 1-based, but this is 0-based for
the user). */
  @ApiProperty({ description: `0-based index for the partial image (backend is 1-based, but this is 0-based for
the user).`, type: 'number' })
  @IsNumber()
  ...
  @ApiProperty({ description: `The type of the event. Always 'response.image_generation_call.partial_image'.`, example: 'response.image_generation_call.partial_image' })
