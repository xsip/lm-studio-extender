...
export class ResponseFileSearchCallInProgressEventDto {
  ...
  @ApiProperty({ description: `The index of the output item that the file search call is initiated.`, type: 'number' })
  @IsNumber()
  output_index!: number;
  ...
  @ApiProperty({ description: `The type of the event. Always \`response.file_search_call.in_progress\`.`, example: 'response.file_search_call.in_progress' })
