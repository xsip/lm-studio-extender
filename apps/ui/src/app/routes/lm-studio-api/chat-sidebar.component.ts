...
@Component({
  ...
  template: `
    ...
    >
      ...
      >
        <span
          class="text-[10px] text-text-muted uppercase tracking-[0.12em] font-semibold"
          style="letter-spacing:0.12em;"
          >{{ 'sidebar.history' | translate }}</span
        >
        @if (chatsLoading()) {
          <app-spinner />
        }
      ...
      <div class="flex-1 overflow-y-auto py-1 min-h-0">
        ...
        >
          ...
          <div class="text-text-muted mt-0.5 text-[10px] font-mono truncate">
            {{ 'sidebar.newChatSubtitle' | translate }}
          </div>
        </button>
              ...
              <div class="flex items-center gap-1.5">
                @if (chat.useCrypto) {
                  <svg
                    ...
                    stroke-width="2"
                    viewBox="0 0 24 24"
                    [attr.aria-label]="'sidebar.encrypted' | translate"
                  >
                    <path
    ...
    @if (ctxMenu(); as menu) {
      ...
      ></div>
      <div
        class="fixed z-50 w-52 bg-surface-raised border border-border-default rounded-xl overflow-hidden py-1"
