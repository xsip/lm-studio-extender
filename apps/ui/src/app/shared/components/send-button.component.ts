  ...
  template: `
    ...
    >
      <span
        ...
        style="background: linear-gradient(135deg, transparent 30%, rgba(255,255,255,0.10) 50%, transparent 70%);"
      ></span>
      @if (streaming()) {
        <span
          class="w-3 h-3 rounded-full border-2 border-white/30 border-t-white animate-spin shrink-0"
        ></span>
        <span>{{ 'sendButton.streaming' | translate }}</span>
      } @else {
        <ng-icon
          name="heroPaperAirplane"
          class="w-3.5 h-3.5 transition-all duration-200 group-hover:translate-x-0.5 animate-scale-in"
        />
