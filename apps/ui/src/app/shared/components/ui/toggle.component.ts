...
@Component({
  ...
  template: `
    <button
      ...
      [disabled]="isDisabled"
      (click)="toggle()"
      class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/50 disabled:opacity-50 disabled:cursor-not-allowed"
      [class]="checked ? activeColor() : 'bg-zinc-700'"
    >
      <span
